% Presentation: Solving Takuzu via QAOA on a Native HUBO Formulation
...
\author[E. Yurtsever]{Ege Yurtsever}
\institute{Sabancı University}
\date{2026}
\venue{Faculty of Engineering and Natural Sciences}
...
\framesubtitle{Why Takuzu? Why Quantum?}
...
\begin{tikzpicture}[scale=0.8]
    % Draw 4x4 Takuzu grid
    \draw[thick] (0,0) grid (4,4);
    % Fill cells with a valid solution
...
\framesubtitle{Rules and Complexity}
...
The Takuzu decision problem is \textbf{NP-complete} \cite{takuzunp2016}, reduced from Not-All-Equal 3-SAT.
\end{alertblock}
...
\framesubtitle{Optimization Formulations}

\begin{columns}
\begin{column}{0.48\textwidth}
\begin{block}{HUBO (Higher-Order)}
$$f(\mathbf{x}) = \sum_{S} c_S \prod_{i \in S} x_i$$
\begin{itemize}
    \item Arbitrary degree polynomials
    \item Natural for complex constraints
    \item QAOA can handle natively
\end{itemize}
\end{block}
\end{column}
...
\begin{itemize}
    \item Degree $\leq 2$ only
    \item Required by quantum annealers
    \item Needs auxiliary variables for reduction
\end{itemize}
\end{block}
\end{column}
\end{columns}
...
QAOA operates on cost Hamiltonians of arbitrary locality \cite{hadfield2019qaoa}. No need to reduce HUBO $\rightarrow$ QUBO.
\end{exampleblock}
\end{frame}

\begin{frame}{QAOA Overview}
\framesubtitle{Quantum Approximate Optimization Algorithm \cite{farhi2014qaoa}}
...
\begin{itemize}
    ...
    \item Depth $p$ controls expressibility
\end{itemize}
\end{frame}
...
\framesubtitle{Three Constraint Types}

Variables: $x_{r,c} \in \{0,1\}$ for each cell. Total: $N^2 = 16$ for $N=4$.

\bigskip
\textbf{1. Consecutive Limit} (degree 2 after simplification):
$$H_{C1}: \quad x_i x_{i+1} x_{i+2} + (1-x_i)(1-x_{i+1})(1-x_{i+2})$$

\bigskip
\textbf{2. Cardinality} (degree 2):
$$H_{C2}: \quad \left(\sum_{c=1}^N x_{r,c} - N/2\right)^2$$

\bigskip
\textbf{3. Uniqueness} (degree $2N = 8$ for $N=4$):
$$H_{C3}: \quad \prod_{c=1}^N \left(2x_{r,c}x_{s,c} - x_{r,c} - x_{s,c} + 1\right)$$
\end{frame}

\begin{frame}{Complete HUBO Objective}
\framesubtitle{Combining All Constraints}
...
Substitution $x_i \rightarrow (I - Z_i)/2$ converts each HUBO monomial into weighted Pauli-$Z$ strings for the cost Hamiltonian.
...
\framesubtitle{Quadratization Overhead}

Rosenberg's method \cite{rosenberg1975reduction} reduces degree by introducing auxiliary variables:
...
Native HUBO: 16 qubits. QUBO: 212 qubits. Simulation of QUBO variant is intractable.
\end{alertblock}
\end{frame}
...
\framesubtitle{Tools, Parameters, and Methods}
...
\begin{itemize}
    \item PySAT library \cite{ignatiev2019rc2}
...
\bigskip
\textbf{Grid size limited to $N=4$}: statevector requires $2^{16}$ amplitudes; $N=6$ (36 qubits) exceeds classical simulation capacity.
\end{frame}
...
\begin{frame}{QAOA Results}
\framesubtitle{9/9 Trials Found Valid Solutions}
...
\begin{frame}{Comparison with Classical Solver}
\framesubtitle{QAOA vs. RC2 SAT}
...
\framesubtitle{Analysis and Interpretation}
...
\begin{enumerate}
    ...
    \bigskip
    \item \textbf{High degeneracy helps.} 72 ground states out of 65,536 ($\approx 0.11\%$ density) makes sampling feasible.
    
    \bigskip
    \item \textbf{Classical dominates at small scale.} RC2 solves in sub-ms; quantum advantage requires larger instances and real hardware.
\end{enumerate}
\end{frame}
...
\bigskip
\begin{exampleblock}{Code Availability}
...
\bibitem{farhi2014qaoa} E. Farhi et al., ``A Quantum Approximate Optimization Algorithm,'' arXiv:1411.4028, 2014.
\bibitem{takuzunp2016} M. Berthier, ``The complexity of Takuzu puzzles,'' \textit{Theoretical Computer Science}, vol. 654, 2016.
\bibitem{rosenberg1975reduction} I. G. Rosenberg, ``Reduction of bivalent maximization to the quadratic case,'' 1975.
\bibitem{hadfield2019qaoa} S. Hadfield et al., ``From QAOA to a Quantum Alternating Operator Ansatz,'' \textit{Algorithms}, 2019.
\bibitem{ignatiev2019rc2} A. Ignatiev et al., ``RC2: An efficient MaxSAT solver,'' \textit{JSAT}, 2019.
\bibitem{lucas2014ising} A. Lucas, ``Ising formulations of many NP problems,'' \textit{Frontiers in Physics}, 2014.
\end{thebibliography}
\end{frame}
